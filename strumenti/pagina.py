...
offerte = [dict(cat=o.cat, ins=o.ins, rep=o.rep, pro=o.pro, fmt=o.fmt, prezzo=o.prezzo,
           ...
           for o in OFFERTE]
# Le marche proprie dei discount non si scrivono: l'elenco sta in
# marche_proprie.py, perché serve anche alla pagina dei prezzi (prezzi.py).
from marche_proprie import MARCHE_PROPRIE
for _o in offerte:
    _marca, _nome, _agg = titolo(_o['pro'], _o['fmt'], _o['bolli'])
    if _marca in MARCHE_PROPRIE.get(_o['ins'], ()):
        _o['propria'] = _marca
    elif _marca:
        _o['marca'] = _marca
    if _nome != _o['pro']:
...
/* Dal 2026-09-24 il benvenuto è un fumetto (vedi benvenuto()): sta sopra il
   suo disegno, il testo centrato, un po' più grande di quelli della mappa. */
.benvenuto{position:relative;z-index:1}
.fumetto.f-bv{--fs:15.5px;width:86%;max-width:360px;text-align:center}
.fumetto.f-bv b{font-size:1.2em;margin-bottom:6px}
.fumetto.f-bv span{display:block;margin-top:3px}
...
.fumetto{padding:1.05em 1.2em 1.1em;font-size:var(--fs,14px);line-height:1.3;color:#27231F;
  /* Il testo non si seleziona (Manlio, 2026-09-26: «il testo è selezionabile e
     crea ulteriori problemi»): un dito che resta giù un attimo non deve
     accendere la selezione né il menù «Copia». */
  -webkit-user-select:none;user-select:none;-webkit-touch-callout:none;
  -webkit-tap-highlight-color:transparent}
.mappa-aiuto .fumetto{cursor:pointer}
.mappa-aiuto .fumetto:focus{outline:none}
.mappa-aiuto .fumetto:focus-visible{outline:2px solid #27231F;outline-offset:2px;border-radius:14px}
...
/* il fumetto di benvenuto: verde come «Prodotti», un po' trasparente */
#frecce-aiuto .p-bv{fill:#DDF1E2;fill-opacity:.6;stroke:#3E9A5E;stroke-opacity:.8}
...
/* I QUATTRO TASTI DEL MENÙ SONO PILLOLE, COME QUELLI DEI PRODOTTI (Manlio,
   2026-09-23 sera: «per omogeneità, se i tasti sono su una pillola, perché i
   tasti principali no?»). Stessa griglia da quattro della griglia dei prodotti
   qui sopra, stesso bordo, stesso fondo, angoli tondi; l'icona resta sopra la
   scritta. Quello della sezione in cui si è è rosso pieno, come il prodotto
   acceso. */
#riga-cerca{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px;
...
let scelto = -1;
/* LA MAPPA DELL'AIUTO ALL'APERTURA (Manlio, 2026-09-24, con un disegno:
   «questa immagine di Help ci sia aprendo il sito, e basta: che non si
   arrivi a una pagina con già dei prodotti»). Finché è vera, al posto delle
   offerte c'è la mappa coi fumetti e le frecce verso i tasti, e la griglia
   dei prodotti sta giù. Diventa falsa al primo tasto del menù; il titolo
   «Spesa» la rimette. */
let inizio = true;
const mappaVisibile = () => inizio && scelto < 0 && !ricercaAperta && !personaleAperto && !cassettoAperto;
const grigliaGiu = () => ricercaAperta || personaleAperto || mappaVisibile();
...
function disegnaTasti() {
  ...
  pro.setAttribute('aria-pressed', String(!ricercaAperta && !personaleAperto && !mappaVisibile()));
  pro.onclick = () => {
    /* Dalla mappa dell'aiuto «Prodotti» fa salire la griglia: lì si comincia. */
    if (mappaVisibile()) { inizio = false; disegna(); return; }
    vaiInizio();
  };
  suo.appendChild(pro);
...
function apriPersonale(si) {
  /* Lasciando la mappa dell'aiuto si rifà la pagina dei prodotti che sta
     sotto (se no, tornando, ci si ritrova la mappa) e si tolgono le frecce. */
  const daMappa = si && mappaVisibile();
  if (si) inizio = false;
  if (si) {
  ...
  frecceAiuto();   // il fumetto di benvenuto sparisce o torna con la sua pagina
}
...
function apriRicerca(si, senzaFuoco) {
  const daMappa = si && mappaVisibile();   // come in apriPersonale
  if (si) inizio = false;
  if (si && personaleAperto) apriPersonale(false);
  ...
  frecceAiuto();   // come in apriPersonale
}
...
function disegnaMarche() {
  ...
  /* Col pannello aperto (ricerca, grandi marche o un volantino) non si
     ...
     marche). */
  document.querySelector('.barra').classList.toggle('giu', grigliaGiu());
...
function benvenuto() {
  ...
  /* UN FUMETTO COME QUELLI DELLA MAPPA, un po' trasparente, con la coda
     verso i prodotti (Manlio, 2026-09-24: «trasformare anche questo in una
     nuvoletta, dove si dicano le stesse cose in forma più concisa, e si
     ricordi che toccando le schede esce la foto del volantino»). La forma
     la disegna frecceAiuto(), come per la mappa. */
  d.innerHTML = '<div class="fumetto f-bv"><b>Tocca un prodotto qui sotto</b>'
    + '<span>Escono le sue offerte, dalla più conveniente.</span>'
    + '<span>Tocca un\u2019offerta per vedere la foto del volantino.</span>'
    + '<span>«Organizza i prodotti» sceglie quali tenere.</span></div>';
  return d;
}

/* LA MAPPA DELL'AIUTO: un titolo, e cinque fumetti con la freccia verso
   quello che spiegano (i due pallini in alto e i quattro tasti del menù).
   I fumetti stanno nella pagina; le frecce in un disegno sopra, fermo sullo
...
const FUMETTI = [
  { dove: 'pallini', tasto: '#apri-config', cls: 'f-alto', tinta: 'p-alto', coda: 'dx', t: 'In alto',
...
function mappaAiuto() {
  ...
  FUMETTI.forEach((f, i) => {
    ...
    b.querySelector('b').textContent = f.t;
    b.querySelector('span').textContent = f.x;
    /* TOCCARE IL FUMETTO È COME TOCCARE IL SUO TASTO (Manlio, 2026-09-26:
       «molte persone fanno clic sui fumetti invece che sui tasti»). Il
       fumetto «In alto» parla di due pallini: apre l'ingranaggio, il primo
       che nomina. */
    b.setAttribute('role', 'button');
    b.tabIndex = 0;
    const premi = () => { const t = document.querySelector(f.tasto || f.dove); if (t) t.click(); };
    b.addEventListener('click', premi);
    b.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); premi(); } });
    d.querySelector(righe[f.cls]).appendChild(b);
  });
  return d;
}
/* I PALLONCINI (Manlio, 2026-09-24, con un disegno: «nuvolette più
   organiche»): fumetto e freccia sono una forma sola, un palloncino tondo e
   un po' storto che si stringe in una coda lunga e finisce a punta sul suo
   tasto. Il testo sta nella pagina; il disegno sta sotto, fermo sullo
   schermo, e si rifà quando la pagina scorre o cambia misura. Mai rosso:
   toccare un palloncino fa quello che fa il suo tasto, ma il «premi qui»
   resta il tasto. */
function liscia(pt) {
  /* Una curva morbida che passa per tutti i punti (Catmull-Rom). */
  let d = '';
  for (let i = 0; i < pt.length - 1; i++) {
    const p0 = pt[Math.max(i - 1, 0)], p1 = pt[i], p2 = pt[i + 1], p3 = pt[Math.min(i + 2, pt.length - 1)];
...
function palloncino(a, z, su, seme, f) {
  ...
  /* Il pallone: tondo ma non un'ellisse (ci deve stare il testo), un po'
     più pieno dalla parte opposta alla coda, e storto ognuno a modo suo. */
  const M = 144, pts = [];
  for (let i = 0; i < M; i++) {
    const t = i / M * 2 * Math.PI, c = Math.cos(t), si = Math.sin(t);
    const w = 1.1 - .03 * s * si + .02 * Math.sin(2 * t + seme * 1.7) + .012 * Math.sin(3 * t + seme * 2.3);
    pts.push([cx + rx * w * Math.sign(c) * Math.pow(Math.abs(c), 2 / 2.5),
              cy + ry * w * Math.sign(si) * Math.pow(Math.abs(si), 2 / 2.5)]);
  }
  ...
  /* Col bersaglio largo (la griglia dei prodotti, per il fumetto di
     benvenuto: «sotto») la punta scende quasi dritta, piegando appena verso
     l'esterno come una virgola: puntata al centro, la coda si attorcigliava. */
  const zx = f.sotto ? x0 + (f.coda === 'sx' ? -10 : 10)
    : f.lato === 'sx' ? z.left + z.width * .22 : f.lato === 'dx' ? z.right - z.width * .22 : (z.left + z.right) / 2;
  const nw = Math.min(20, a.width * .15);
  const bocca = [];
  pts.forEach((p, i) => { if (s * (p[1] - cy) > 0 && Math.abs(p[0] - x0) < nw) bocca.push(i); });
  if (bocca.length < 2) return '';
  const primo = Math.min(...bocca), ultimo = Math.max(...bocca);
  const arco = [];
...
/* Il fumetto della pagina di benvenuto: la coda va alla griglia dei
   prodotti, qui sotto. */
const FUMETTO_BV = { dove: '.barra', tinta: 'p-bv', coda: 'sx', quota: .2, sotto: true };
function frecceAiuto() {
  let svg = svgAiuto;
  const mappa = mappaVisibile() && document.querySelector('.mappa-aiuto');
  const bv = !mappa && scelto < 0 && !ricercaAperta && !personaleAperto && !cassettoAperto
    && document.querySelector('#risultato .benvenuto .fumetto');
  if (!mappa && !bv) { if (svg) svg.remove(); svgAiuto = null; return; }
  if (!svg) {
  ...
  const coppie = mappa
    ? [...document.querySelectorAll('.mappa-aiuto .fumetto')].map(b => [b, FUMETTI[+b.dataset.i], +b.dataset.i + 1])
    : [[bv, FUMETTO_BV, 7]];
  coppie.forEach(([b, f, seme]) => {
    const bersaglio = f.dove === 'pallini' ? document.querySelector('header .pallini') : document.querySelector(f.dove);
    const a = r(b);
    let z = r(bersaglio);
    if (!a || !z || !a.width || !z.width) return;
    /* La griglia dei prodotti, appena toccato «Prodotti», sta ancora salendo:
       la coda punta a dove arriverà, non a dove è in quel momento. */
    if (f.dove === '.barra') {
      try {
        const m = new DOMMatrix(getComputedStyle(bersaglio).transform);
        z = { left: z.left, right: z.right, width: z.width, top: z.top - m.f, bottom: z.bottom - m.f };
      } catch (e) {}
    }
    const su = z.bottom <= a.top;   // il tasto sta sopra (i pallini) o sotto (il menù)
    const d = palloncino(a, z, su, seme, f);
    if (!d) return;
