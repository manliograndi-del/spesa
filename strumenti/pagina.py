...
h1 .info{vertical-align:middle;margin-left:8px}
...
.capo .info{flex:none}
/* La crocetta per togliere un prodotto: piccola, ma con dentro un bersaglio da
   dito. Il bordo la fa vedere come un tasto, il rosso solo quando la si tocca. */
.croce{flex:none;margin-left:auto;width:34px;height:34px;border-radius:9px;
  border:1.5px solid var(--linea-forte);background:var(--carta);color:var(--tenue);
  font-size:19px;line-height:1;cursor:pointer;padding:0;display:grid;place-items:center}
.croce:hover{border-color:var(--rosso);color:var(--rosso)}
...
<div class="guscio">
<header>
  <h1><span>Torino · Corso Siracusa</span>La lista della spesa
    <button type="button" class="info" aria-expanded="false"
            aria-label="Come funziona questa pagina">i</button></h1>
...
function disegna() {
  ...
  capo.innerHTML = '<h2></h2>'
    ...
    + '<button type="button" class="croce" aria-label="Togli dalla lista">\u00d7</button>';
  capo.querySelector('h2').textContent = v.nome;
  capo.querySelector('.croce').setAttribute('aria-label', 'Togli «' + v.nome + '» dalla lista');
  ...
  conferma.innerHTML = '<span></span><button type="button" class="si">Togli</button>'
    + '<button type="button" class="no">Lascia</button>';
  conferma.querySelector('span').textContent = 'Tolgo «' + v.nome + '»?';
  ...
  capo.querySelector('.croce').onclick = () => { conferma.hidden = !conferma.hidden; };
